...
var path_json = "minorhythm/json";
var dirs = fs.readdirSync(path_json);
dirs.alphanumSort();
//console.log(dirs);
var blogs = [];
for(var i = 0; i < dirs.length; i++) {
	console.log(dirs[i]);
	var jsons = fs.readdirSync(path_json + "/" + dirs[i]);
	jsons.alphanumSort();
	for(var j = 0; j < jsons.length; j++) {
		var json = JSON.parse(fs.readFileSync(path_json + "/" + dirs[i] + "/" + jsons[j]));
		var blog = {};
		var id = jsons[j].replace(/\.json$/g,"");
...
fs.writeFileSync(path_json + "/index.json", JSON.stringify(blogs));
console.log("done");
